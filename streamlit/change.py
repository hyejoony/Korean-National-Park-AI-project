...
def display_map_with_draw(m):
    ...
    # 지도 객체를 생성할 때 기본 레이어로 설정
    m = folium.Map(location=[37.5665, 126.9780], zoom_start=12, tiles='V-World Satellite')
...
# 최근 다이나믹 월드 데이터 가져오기
def get_dynamic_world_data(aoi, recent_date):
    # 다이나믹 월드 이미지 컬렉션을 필터링하여 해당 날짜의 이미지를 가져옵니다.
    dw_images = ee.ImageCollection('GOOGLE/DYNAMICWORLD/V1') \
            .filterBounds(aoi) \
            .filterDate(ee.Date(recent_date).advance(-5, 'day'), ee.Date(recent_date)) \
            .select(['built', 'crops', 'water', 'trees', 'grass', 'bare'])
    
    # 해당 기간의 이미지들의 평균을 구하고, 투영 방식과 해상도를 설정합니다.
    recent_mean_image = dw_images.mean().reproject(crs='EPSG:4326', scale=10)
    
    # 지정된 영역에 대한 평균값을 계산합니다.
    recent_mean_values = recent_mean_image.reduceRegion(
        reducer=ee.Reducer.mean(),
        geometry=aoi,
        scale=10
    )
    ...
    # 평균값을 각 변수로 가져옵니다.
    built_mean = recent_mean_values.get('built').getInfo()
    crops_mean = recent_mean_values.get('crops').getInfo()
    water_mean = recent_mean_values.get('water').getInfo()
    trees_mean = recent_mean_values.get('trees').getInfo()
    grass_mean = recent_mean_values.get('grass').getInfo()
    bare_mean = recent_mean_values.get('bare').getInfo()

    return built_mean, crops_mean, water_mean, trees_mean, grass_mean, bare_mean


# 과거 평균확인 코드
def get_dynamic_world_data2(aoi):
    start_date = '2014-01-01'
    end_date = '2022-12-31'
...
# 가장 가까운 날짜 찾기 SAR
def find_nearest_date_SAR(aoi, target_date, collection_id):
...
# 가장 가까운 날짜 찾기 다이나믹 월드
def find_nearest_date_dynamic_world(aoi, target_date, collection_id):
    # 관심 지역(AOI)에 따라 특정 Dynamic World 컬렉션을 필터링합니다.
    collection = ee.ImageCollection(collection_id).filterBounds(aoi)
    
    # 컬렉션 내에서 가능한 이미지 날짜 리스트를 가져옵니다.
    available_dates = collection.aggregate_array('system:time_start').getInfo()
    available_dates = [datetime.utcfromtimestamp(date / 1000) for date in available_dates]
    
    # 가능한 날짜가 없으면 None을 반환합니다.
    if not available_dates:
        return None
    
    # 타겟 날짜와 가장 가까운 날짜를 찾습니다.
    nearest_date = min(available_dates, key=lambda d: abs(d - target_date))
    return nearest_date
...
def process_user_input(lat, lon, variables):
    ...
    nearest_date_vh = find_nearest_date_SAR(aoi, current_date, 'COPERNICUS/S1_GRD')
    # Dynamic World 데이터셋에 대한 최근 날짜 찾기
    nearest_date_dw = find_nearest_date_dynamic_world(aoi, current_date, 'GOOGLE/DYNAMICWORLD/V1')
    # 지정된 기간 동안의 평균값
    period_mean_vh = get_sentinel_1_mean(aoi)
    period_built_mean, period_crops_mean, period_water_mean, period_trees_mean, period_grass_mean, period_bare_mean = get_dynamic_world_data2(aoi)
...
# 'VH'가 variables에 있고, recent_data에 'VH' 값이 'None'이 아닌 경우 출력
    if 'VH' in variables and 'VH' in recent_data and recent_data['VH'][0] != 'None':
